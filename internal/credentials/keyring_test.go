...
func (s *KeyringCredentialsTestSuite) TestMigrationFromLegacy() {
	...
	// Set debug log expectations - this is needed in list() call rather than in save()
	s.log.On("Debug", "Failed to delete legacy credentials entry", "error", "secret not found").Maybe().Return()

	// Now try to load the credentials - this should trigger migration
	creds, err := cs.List()
	...
	// Sleep briefly to allow the background deletion of the legacy key to complete
	time.Sleep(100 * time.Millisecond)

	// Check that the legacy key was deleted
	_, err = keyring.Get(ServiceName, LegacyKey)
	s.Equal(keyring.ErrNotFound, err, "Legacy key should be deleted after migration")

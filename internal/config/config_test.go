...
func (s *ConfigSuite) TestInitialComments() {
	configFile := GetConfigPath(s.cwd, "myConfig")
	cfg := New()
	err := cfg.WriteFile(configFile)
	s.NoError(err)
	...
	contents, err := configFile.ReadFile()
	s.NoError(err)
	s.True(bytes.HasPrefix(contents, []byte("# Configuration file generated by Posit Publisher.\n# Please review and modify as needed.\n")))
}
...
func (s *ConfigSuite) TestLeadingComments() {
	configFile := GetConfigPath(s.cwd, "myConfig")

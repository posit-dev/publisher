...
func (s *AppTypesSuite) TestAppMode() {
	for _, each := range []struct {
		...
		comment := fmt.Sprintf("AppMode=%d (%s)", each.Mode, AppModeToString(each.Mode))
		...
		s.Equal(each.TensorFlowModelAPI, each.Mode.IsTensorFlowModelAPI(), comment)
	}
}
...
func (s *AppTypesSuite) TestAppModeStrings() {
	...
	for _, each := range []struct {
		...
		comment := fmt.Sprintf("AppMode=%d (%s)", each.Mode, each.String)
		actual, err := AppModeFromString(each.String)
		s.Nil(err, comment)
		s.Equal(each.Mode, actual, comment)
		s.Equal(each.String, AppModeToString(each.Mode), comment)
	}
	...
	s.Equal("unknown", AppModeToString(UnknownMode))
}
...
func (s *AppTypesSuite) TestDescription() {
	for _, each := range []struct {
		...
		{TensorFlowModelAPI, "TensorFlow model"},
		...
		comment := fmt.Sprintf("AppMode=%d", each.Mode)

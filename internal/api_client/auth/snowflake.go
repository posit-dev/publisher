...
const (
	...
	user           = "chris@posit.co"
	privateKeyFile = "/Users/chris/.snowflake/rsa_key.p8"

	// endpoint = "b3coiaib-duloftf-posit-software-pbc-dev.snowflakecomputing.app"
	// endpoint = "f3eoiaib-duloftf-posit-software-pbc-dev.snowflakecomputing.app"
)

func NewSnowflakeAuthenticator(connectionName string) (AuthMethod, error) {
	if connectionName != "" && connectionName != "default" {
		return nil, fmt.Errorf("invalid snowflake connection name")
	}

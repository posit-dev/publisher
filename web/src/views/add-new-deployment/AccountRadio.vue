...
<template>
  ...
  >
    <q-item-section avatar top class="col-1">
      <q-radio v-model="radioModel" name="account" :val="account.name" />
    </q-item-section>
    <q-item-section class="q-ml-md">
      <q-item-label>
        {{ account.name }}
      </q-item-label>
      <q-item-label caption class="account-caption">
        Account: {{ calculateName(account) }}
      </q-item-label>
      <q-item-label caption class="account-url">
        URL: {{ account.url }}
      </q-item-label>
      <q-item-label caption class="q-pt-sm account-credential">

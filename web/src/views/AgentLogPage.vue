...
<template>
  <div class="publisher-layout q-py-md">
    <q-breadcrumbs>
      ...
      <q-breadcrumbs-el label="Agent Log" />
    </q-breadcrumbs>
    <div class="q-pt-lg">
      <q-checkbox
        v-model="showDebug"
        ...
        left-label
      />
      <template v-for="logMsg in eventStore.agentLogs" :key="logMsg">
        <template
          v-if="
            logMsg.data.level !== 'DEBUG' ||
            (logMsg.data.type !== 'DEBUG' && showDebug)
          "
        >
          <hr />
          <div class="row row-gap-md q-py-sm logMsg">
            ...
            <div class="col-12 col-sm-10">
              <template v-for="key in Object.keys(logMsg.data)" :key="key">
                <div v-if="logMsg.data[key]" class="row">

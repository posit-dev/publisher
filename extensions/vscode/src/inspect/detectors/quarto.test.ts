...
describe("QuartoDetector", () => {
  ...
  test("fallback when quarto binary missing: .md", async () => {
  ...
  test("directory scan excludes .md files without _quarto.yml", async () => {
    setupGlobDir(["README.md", "CHANGELOG.md", "report.qmd"]);
    mockAccess.mockRejectedValue(new Error("ENOENT"));
    mockReadFile.mockResolvedValue("# Content\n");
    ...
    expect(configs).toHaveLength(1);
    expect(configs[0]?.entrypoint).toBe("report.qmd");
  });

  test("directory scan includes .md files when _quarto.yml exists", async () => {
    setupGlobDir(["notes.md", "report.qmd"]);
    mockAccess.mockImplementation((filePath: string) => {
      if (filePath.endsWith("_quarto.yml")) {
    ...
    mockExecFile.mockImplementation((_cmd: string, args: string[]) => {
      const target = args[1] ?? "";
      if (target.endsWith("notes.md")) {
        return Promise.resolve({
          stdout: makeInspectOutput({
            engines: ["markdown"],
            files: { input: ["/project/notes.md"], configResources: [] },
            formats: {
              html: {
    ...
    expect(configs.find((c) => c.entrypoint === "notes.md")).toBeDefined();
    expect(configs.find((c) => c.entrypoint === "report.qmd")).toBeDefined();
  });

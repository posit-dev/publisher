...
function isPythonEnvironmentDir(dirPath: string): boolean {
  return PYTHON_BIN_PATHS.some((bin) => {
    try {
      fs.accessSync(path.join(dirPath, bin));
      return true;
    } catch {
      return false;
    }
...
function shouldInclude(
  ...
  // Append trailing slash for directory matching (gitignore convention)
  const pathForMatch = relativePath;
  ...
  for (const rule of rules) {
    ...
    if (rule.matchesPath(pathForMatch)) {
...
function walkDirectory(
  ...
  for (const entry of dirEntries) {
    ...
    if (isDir) {
      ...
      if (
        isPythonEnvironmentDir(absolutePath) ||
      ...
      walkDirectory(baseDir, absolutePath, rules, entries);
    } else if (isFile) {
...
export function collectFiles(
  ...
  walkDirectory(projectDir, projectDir, rules, entries);
  return entries;
}

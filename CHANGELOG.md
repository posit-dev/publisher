...
- Clicking "Scan" to generate a Python requirements file (or "Set up renv" to create an R lockfile) now also adds that file to your configuration's file list. Before, the file was created but not included, so the next deploy failed with a "Missing dependency file" error until you added it by hand. (#4400)
...
- Fixed `memory_limit` and `memory_request` examples in the publishing configuration schema being quoted as strings instead of integers. (#1505)
